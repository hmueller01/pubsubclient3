...
* `write()` and `write_P()`: replaced byte-by-byte `appendBuffer()` loop with `memcpy`/`memcpy_P` block copy, reducing virtual function call overhead
* `writeBuffer()` with `MQTT_MAX_TRANSFER_SIZE`: moved `_lastOutActivity = millis()` outside chunk loop
* `bufferWritePos`: changed type from `uint16_t` to `size_t` for improved compatibility
* Improved handlePacket comments for clarity (e.g., "QoS 0: no Packet Identifier")

### Fixed

* `connect()`: added null-buffer guard to prevent crash on buffer allocation failure
* `disconnect()`: added null-buffer guard
* `loop()`: added upfront guard for buffer existence and size validation
* `handlePacket()`: added multiple boundary checks for topic length and msgId fields
